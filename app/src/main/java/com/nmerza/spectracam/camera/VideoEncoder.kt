            ...
            if (outIndex == MediaCodec.INFO_TRY_AGAIN_LATER) {
                ...
                if (muxerStarted.get() && info.size > 0 && (info.flags and MediaCodec.BUFFER_FLAG_CODEC_CONFIG) == 0) {
                    ...
                    if (isVideo) {
                        ...
                        // Re-base the timestamp to start from zero and ensure it's monotonic.
                        info.presentationTimeUs = info.presentationTimeUs - firstVideoPtsUs
                        if (info.presentationTimeUs <= lastVideoPtsUs) {
                            info.presentationTimeUs = lastVideoPtsUs + 1
                        }
